...
import { PreviousZReportItem } from "./previousZReportItem";
import {
  DateRangePicker,
...
import { useGetAllZReportsQuery } from "../../services/api";
...
export const PreviousZReports: FC = () => {
  const theme = useTheme();
  const dispatch = useAppDispatch();
  const pageSize = useAppSelector(selectPageSize);
  const cursor = useAppSelector(selectCursor);
  const { after, before } = useAppSelector(selectDateRange);
  const { data, isLoading, isFetching, isError } = useGetAllZReportsQuery({
    pageSize,
    cursor,
    after,
    before,
      ...
      {isLoading || isError || !zReports ? (
        <Box sx={{ width: "100%", height: 300 }}>
          ...
          {isError || (!isLoading && !zReports) ? (
            <NotFound message="No Z Report found." />
          ) : null}
        </Box>
      ) : (
...
export default PreviousZReports;
